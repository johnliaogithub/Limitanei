...
A physics-accurate reinforcement learning environment built in MuJoCo for autonomous drone interception. A PPO agent learns to navigate toward and hit an aerial target while compensating for high-frequency recoil forces generated by an onboard weapon. The project investigates whether a learned control policy can maintain stable flight and accurate target engagement during weapon discharge, a challenging control problem for light-weight, autonomous platforms. See [Setup](#setup) to get started.
